...
- 🤖 **Claude** (자동, GitHub Action) — 지문뿐인 README에 **풀이·접근·복잡도** 보강, **유형 분류**, 위 **인덱스 표** 갱신
- 🙋 **나** — 문제 풀기, **복습 대상 표시(🔁)**

### 작업 흐름
1. SWEA에서 문제를 풀고 제출하면 → 백준허브가 자동으로 폴더·코드·README를 커밋한다.
2. 그 푸시를 GitHub Action이 감지해 Claude를 호출한다.
3. Claude가 문제별 README를 보강하고, 유형을 분류해 인덱스 표에 한 줄을 추가한다.
4. 나는 복습하고 싶은 문제에 🔁만 표시한다.

> 💡 커밋 메시지는 백준허브가 자동 생성하므로(`[D3] Title: ..., Time: ..., -BaekjoonHub`)
> 별도의 커밋 컨벤션은 두지 않습니다.
